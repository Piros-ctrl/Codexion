...
static int	parse_un(char *arg, unsigned int *value, char *name, int positive)
{
	if (is_solid_int(arg))
	{
		printf("Error: %s must be a valid integer.\n", name);
...
{
	if (!parse_un(av[1], &config->number_of_coders, "number_of_coders", 1))
		return (1);
	if (!parse_un(av[2], &config->time_to_burnout, "time_to_burnout", 1))
		return (1);
	if (!parse_un(av[3], &config->time_to_compile, "time_to_compile", 1))
		return (1);
	if (!parse_number(av[4], &config->time_to_debug, "time_to_debug", 0))
		return (1);
	if (!parse_number(av[5], &config->time_to_refactor, "time_to_refactor", 0))
		return (1);
	if (!parse_un(av[6], &config->number_of_compiles_required, "number_of_compiles_required", 1))
		return (1);
